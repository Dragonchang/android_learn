...
#define	VERSION	"2.6"
/*
 * 2.6	: output events buffer log while stopping logger to trigger router thread quit immediately.
...
#define	FILE_PREFIX	"events_" LOG_FILE_TAG
#define	LOG_ID		LOG_ID_EVENTS
...
static int output_events_log (const char *msg)
{
	/*
	 * __android_log_buf_write() cannot be used to output events log or logcat will encounter error, need to use android_bWriteLog() instead.
	 */
	char buf [LOGGER_ENTRY_MAX_PAYLOAD - sizeof (long)];

	if (msg)
	{
		unsigned long len = sizeof (buf) - 1 /* type */ - sizeof (long) /* size field */ - 1 /* new line */; /* == max msg length */

		if (len > strlen (msg)) len = strlen (msg);

		int idx = 0;
		buf [idx] = EVENT_TYPE_STRING;
		idx ++;
		memcpy (& buf [idx], & len, sizeof (len));
		idx += sizeof (len);
		memcpy (& buf [idx], msg, len);
		idx += len;
		buf [idx] = '\n';

		return android_bWriteLog (0 /* /system/etc/event-log-tags */, buf, 1 + sizeof (len) + len + 1);
	}

	return -1;
}

int logevents_init (const char *name, LOGGER_EXTRA_CONFIG **UNUSED_VAR (extra_configs), int *UNUSED_VAR (extra_config_count))
{
	int ret;
...
void logevents_get_log_filepath (const char *UNUSED_VAR (name), char *buffer, int len)
{
	SAFE_SPRINTF (buffer, len, "%s", log_filename);
}

void logevents_clear_log_files (const char *path)
{
	char buffer [32];
...
int logevents_check_file_type (const char *name, const char *filepath)
{
	return logger_common_check_file_type (name, filepath, FILE_PREFIX, log_filename);
}

unsigned long long logevents_get_total_size (const char *UNUSED_VAR (name))
{
	return total_size;
}

int logevents_update_state (const char *name, int state)
{
...
int logevents_start (const char *name)
{
...
int logevents_stop (const char *name)
{
	if (pipefd != -1)
	{
		pipeinfo.pipefd = -1;
	...
	if (router != (pthread_t) -1)
	{
		...
		output_events_log (LOG_TAG ": stop");
	...
	else
	{
		...
		else
		{
			for (ret = 0, pipeinfo.rotated = 1; (ret < 10) && (pipeinfo.rotated != 0 /* this value will be updated by router thread */); ret ++)
			{
				...
				output_events_log (LOG_TAG ": rotate");
...
int logevents_rotate_and_limit (const char *name, int state, unsigned long rotate_size_mb, unsigned long limited_size_mb, unsigned long reserved_size_mb)
{
