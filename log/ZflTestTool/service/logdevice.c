...
#define	VERSION	"2.8"
/*
 * 2.8	: output main buffer log while stopping logger to trigger router thread quit immediately.
 * 2.7	: support new logger control service.
 * 2.6	: check also missing logging file.
 * 2.5	: share log session with other loggers.
 * 2.4	: remove socket interface.
 * 2.3	: do not use functions would call malloc() or free() in child process.
 * 2.2	: dump logs with threadtime.
 * 2.1	: add storage code to identify the log path.
 * 2.0	: support sn.
 * 1.9	: keep local logs.
 * 1.8	: support auto select log path.
 * 1.7	: use session structure.
 * 1.6	: create timestamp list.
 * 1.5	: set default path to LOG_DIR.
 * 1.4	: add external apis for logctl.
 * 1.3	: remove key thread.
 * 1.2	: change default value by SQA's request.
 * 1.1	: merge dumpkey service.
 */

#define	FILE_PREFIX	"device_" LOG_FILE_TAG
...
int logdevice_init (const char *name, LOGGER_EXTRA_CONFIG **UNUSED_VAR (extra_configs), int *UNUSED_VAR (extra_config_count))
{
	int ret;
...
void logdevice_get_log_filepath (const char *UNUSED_VAR (name), char *buffer, int len)
{
	SAFE_SPRINTF (buffer, len, "%s", log_filename);
}

void logdevice_clear_log_files (const char *path)
{
	char buffer [32];
...
int logdevice_check_file_type (const char *name, const char *filepath)
{
	return logger_common_check_file_type (name, filepath, FILE_PREFIX, log_filename);
}

unsigned long long logdevice_get_total_size (const char *UNUSED_VAR (name))
{
	return total_size;
}

int logdevice_update_state (const char *name, int state)
{
...
int logdevice_start (const char *name)
{
...
int logdevice_stop (const char *name)
{
	if (pipefd != -1)
	{
		pipeinfo.pipefd = -1;
...
int logdevice_rotate_and_limit (const char *name, int state, unsigned long rotate_size_mb, unsigned long limited_size_mb, unsigned long reserved_size_mb)
{
